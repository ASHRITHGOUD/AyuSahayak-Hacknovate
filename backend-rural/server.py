...
def check_answer_relevance_with_gemini(question, answer, original_symptoms):
    prompt = f"""
You are a strict medical triage guardrail AI.
Decide if the user's answer is relevant to the given follow-up medical question.

Rules:
1) If NOT answering the question → is_relevant=false.
2) Chit-chat → false.
3) Clarifications of existing symptoms → allowed.
4) Valid short medical answers → allowed.
5) When unsure → is_relevant=false.

Return STRICT JSON ONLY:
{{
  "is_relevant": true|false,
  "introduces_new_symptom": true|false,
  "reason": "short reason"
}}

QUESTION: "{question}"
ANSWER: "{answer}"
ORIGINAL_SYMPTOMS: {list(original_symptoms)}
JSON:
"""
    ...
    if not parsed:
        # allow natural progression words
        progression_words = ["worse", "better", "improved", "same", "worsened", "unchanged"]
        if any(p in answer.lower() for p in progression_words):
            return {"is_relevant": True, "introduces_new_symptom": False, "reason": "progression ok"}
    
        # allow common medical descriptors
        if re.search(r"\b(\d+ days?|since|severe|mild|night|day|after meals)\b", answer.lower()):
            return {"is_relevant": True, "introduces_new_symptom": False, "reason": "heuristic ok"}
    
        return {"is_relevant": False, "introduces_new_symptom": False, "reason": "parse fail"}
    
    return {
        "is_relevant": bool(parsed.get("is_relevant", False)),
        "introduces_new_symptom": False,
        "reason": parsed.get("reason", "ok")
    }
...
async def next_question(payload: AnswerInput):
    ...
    if not user_answer:
        return {"done": False, "next_question": current_q, "warning": "⚠️ Please answer the question"}

    # ✅ Guardrail
    guard = check_answer_relevance_with_gemini(
        question=current_q,
        answer=user_answer,
    ...
    if not guard["is_relevant"]:
        return {"done": False, "next_question": current_q,
                "warning": "⚠️ That doesn't answer the question."}
